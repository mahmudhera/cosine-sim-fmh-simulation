...
if __name__ == "__main__":
    ...
    a, b = min(df['max_val_v'].tolist()), max(df['max_val_v'].tolist())

    for s, coord in zip([0.0001, 0.001, 0.01, 0.1], [ [0,0], [0,1], [1,0], [1,1] ]):
        df2 = df[ (df['s'] == s) ]
        axs[coord[0],coord[1]].scatter(df2['max_val_v'], 100*(df2['cos_org']-df2['cos_fmh'])/df2['cos_org'], s=1, color='#0077b6')
        axs[coord[0],coord[1]].plot( [a, b], [0, 0], color='#03045e')
        axs[coord[0],coord[1]].set_title(f'FMH Scale Factor {s}', fontsize=11)

    for ax in axs.flat:
        ax.set(xlabel='Num elements in one set', ylabel='% diff from true cosine')
